# docker_develop_for_opensuse
DockerでopenSUSE開発環境を構築する。
...
## コンテナIPアドレスの設定
`.env`内のIPアドレスを任意の値に変更する。(例：127.0.0.8)
<br>
...
2. プルダウンを`Containers`に変更し、コンテナ一覧に表示されている`openSUSE`にマウスオーバーする。
3. `openSUSE`右に表示される`フォルダアイコン`を押下する。
